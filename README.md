...
DDD 基座、标准服务实现与网关参考应用的聚合工程。

本仓库承载可运行的「实现」：通用基础组件、一个完整的标准服务（Auth / RBAC / Customer / Channel）与一个网关参考应用。
...
│   ├── ddd-base-bom/              # 汇总 BOM（第三方 + 基础组件版本）
│   └── ian-ddd-coverage/          # 分布式 E2E 覆盖率基础设施
├── ian-ddd-archetype-std/         # 标准服务实现：Auth / RBAC / Customer / Channel（HTTP 8091）
...
- [ian-ddd-archetype-std/README.md](ian-ddd-archetype-std/README.md) — 标准服务能力、安全边界、ID 生成与本地启动
...
| Dubbo               | 3.3.6   | Triple 协议，网关以 `@DubboReference` 调用标准服务 |
...
│  调用方   │ ──────────────▶ │ ian-ddd-gateway │ ───────────────▶ │ ian-ddd-archetype-std     │
...
- 网关只做 HTTP 接入与认证转发，不连接 Redis、不保存会话；Opaque Token 与登录风控由标准服务的 Auth 统一持有。
- 标准服务同时以 Dubbo Provider 注册（应用名 `ian-frame-archetype-std`，Triple 端口 50051）并暴露 HTTP（8091）。
...
mvn -B -f ian-ddd-archetype-std/pom.xml clean verify
```
...
启动 Nacos、Redis、MySQL 后，按需执行 `ian-ddd-archetype-std/docs/dev-ops/environment/sql/` 下的初始化脚本：
...
单服务门槛当前分档：基础组件与网关 60%，标准服务 `domain` 35%、`infrastructure` 15%，`trigger` / `api` 暂无门槛。
...
| 仓库                              | 内容                                            | 关系                                                         |
|-----------------------------------|-------------------------------------------------|--------------------------------------------------------------|
| [`ddd`](https://github.com/IanGall/ddd)（本仓库） | 基座 + 标准服务实现 + 网关参考应用 | 提供基座制品与参考实现                                       |
