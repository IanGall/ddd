...
- `ContextWebFilter` 必须在应用认证过滤器之后由 Servlet 容器执行。
- 自动配置统一注册 Filter，并设置最低优先级，避免在认证前执行或同一请求执行两次。
- `requestId` 读取并校验 `X-Request-Id`，缺失或非法时生成 UUID，并回写同名响应头。
- `principalName` 只能来自应用注入的 `AuthenticationContextResolver`，禁止信任外部身份 Header。
- 默认解析器返回匿名上下文，所有身份字段为空。
...
- `ContextWebFilter` 禁止直接读取或传播外部 `X-User-Id`、`X-Tenant-Id`、Authorization、Cookie、密码、权限列表和业务对象；应用认证入口可以读取
  Authorization Bearer/Token 完成认证，但凭证原文不得写入 `RequestContext`、日志或 Dubbo Attachment。
...
- 必须覆盖自定义解析器身份注入、外部身份 Header 忽略、请求 ID 生成与回写、匿名请求和请求间隔离。
- 修改后执行 `mvn -q -pl ddd-context-web test`；涉及 Core 协议时执行 `mvn -q install`。
- 自动配置或过滤器顺序变更必须增加对应上下文集成测试。
