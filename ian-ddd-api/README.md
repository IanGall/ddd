...
服务对外与对内的 **API 契约**聚合工程，按「内部 / 外部」分类拆分子模块，独立于任何服务实现。
...
| 维度 | `ian-ddd-api-internal` | `ian-ddd-api-external` |
|------|------------------------|------------------------|
...
> 内部契约的包名保持 `cn.iantech.api.*` 不变（消费方 import 零改动）；外部契约为新命名空间 `cn.iantech.api.external.*`。
...
| `cn.iantech:ian-ddd-api-internal` | 内部 Dubbo RPC 契约 jar |
...
**内部（Dubbo）**：按服务在 `ian-ddd-api-internal` 下扩展接口与 DTO（包 `cn.iantech.api.*`），DTO 必须 `Serializable` + 无参构造。
...
新增子模块时：在 `ian-ddd-api/pom.xml` 的 `<modules>` 登记，并在 `ddd-base-bom` 增加版本管理。
...
- 详细规则见 [ian-ddd-api-internal/agents.md](ian-ddd-api-internal/agents.md) 与 [ian-ddd-api-external/agents.md](ian-ddd-api-external/agents.md)。
