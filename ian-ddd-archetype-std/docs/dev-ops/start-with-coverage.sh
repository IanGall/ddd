#!/usr/bin/env bash
# 带 JaCoCo Agent 启动标准服务（ian-frame-archetype-std），用于分布式 E2E 覆盖率采集。
#
...
COVERAGE_AGENT_NAME="${COVERAGE_AGENT_NAME:-std}"
...
echo "启动标准服务，覆盖率 Agent: ${COVERAGE_AGENT_NAME}@127.0.0.1:${COVERAGE_AGENT_PORT}"

# 自动化测试统一使用测试库：先 dev 再 autotest（后者优先），把 MySQL/Redis 指向测试库
# 见 application-autotest.yml；如需回到 dev 库，设 COVERAGE_SPRING_PROFILES=dev
COVERAGE_SPRING_PROFILES="${COVERAGE_SPRING_PROFILES:-dev,autotest}"

cd "${PROJECT_DIR}/ian-frame-archetype-std-boot"
exec mvn -q spring-boot:run -Pdev \
