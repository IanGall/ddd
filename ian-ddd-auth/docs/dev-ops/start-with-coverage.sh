#!/usr/bin/env bash
# 带 JaCoCo Agent 启动认证服务（ian-ddd-auth），用于分布式 E2E 覆盖率采集。
#
...
COVERAGE_AGENT_NAME="${COVERAGE_AGENT_NAME:-auth}"
...
echo "启动认证服务，覆盖率 Agent: ${COVERAGE_AGENT_NAME}@127.0.0.1:${COVERAGE_AGENT_PORT}"
...
cd "${PROJECT_DIR}/ian-ddd-auth-boot"
